...
public:
  ...
  public:
    using ControlSignature = void(FieldIn pointBatch, FieldIn pointBatchDataOffsets,
      WholeArrayIn keptPointsMask, WholeArrayOut pointsInputToOutput,
  ...
  public:
    using ControlSignature = void(FieldIn cellBatch, FieldOut cellBatchData,
      FieldOut batchWithClippedCellsMask, FieldOut batchWithKeptOrClippedCellsMask,
  ...
  public:
    ...
    using ControlSignature = void(FieldIn cellBatch, FieldIn cellBatchDataOffsets,
      WholeCellSetIn<> cellSet, WholeArrayIn scalars, WholeArrayIn caseIndices,
      WholeArrayOut edges);
  ...
  public:
    ...
    using ControlSignature = void(FieldIn cellBatch, FieldIn cellBatchDataOffsets,
      WholeCellSetIn<> cellSet, WholeArrayIn caseIndices, WholeArrayIn pointMapOutputToInput,
      WholeArrayIn edgeIndexToUnique, WholeArrayOut centroidOffsets,
      WholeArrayOut centroidConnectivity, WholeArrayOut cellMapOutputToInput, WholeArrayOut shapes,
              ...
              if (cellShape != CTI::ST_PNT) // normal cell
              {
                  ...
                  ++pointId, ++index)
                {
                  ...
                  if (pointIndex <= CTI::P7) // Input Point
                  {
                    ...
                    connectivity.Set(
                      cellIndicesOffset, pointMapInputToOutput.Get(points[pointIndex]));
                  }
              ...
              else // cellShape == CTI::ST_PNT
              {
                  ...
                  ++pointId, ++index)
                {
                  ...
                  if (pointIndex <= CTI::P7) // Input Point
                  {
                    ...
                    // assume P0 == 0.  This is why we do not
                    // bother subtracting P0 from pt here.
                    centroidConnectivity.Set(
                      centroidIndicesOffset, pointMapInputToOutput.Get(points[pointIndex]));
                  }
    ...
    vtkm::Float64 value, vtkm::Id batchSize, YamlWriter& log)
  {
    ...
    timer.Start();
    invoke(MarkKeptPoints<Invert>(value), pointBatches, pointBatchesData, batchesWithKeptPointsMask,
      scalars, keptPointsMask);
    timer.Stop();
    ...
    { // A new scope is needed so that batchesWithKeptPointsMaskSelect is released at the end.
      ...
      timer.Start();
      invoke(ComputePointMaps(), batchesWithKeptPointsMaskSelect, pointBatches,
        pointBatchesData, // pointBatchesDataOffsets
        keptPointsMask, pointMapInputToOutput, this->PointMapOutputToInput);
      timer.Stop();
      log.AddDictionaryEntry("seconds-compute-point-maps", timer.GetElapsedTime());
    ...
    timer.Start();
    invoke(ComputeCellStats<Invert>(), cellBatches, cellBatchesData, batchesWithClippedCellsMask,
      batchesWithKeptOrClippedCellsMask, cellSet, keptPointsMask, caseIndices);
    timer.Stop();
    log.AddDictionaryEntry("seconds-compute-cell-stats", timer.GetElapsedTime());
    ...
    { // A new scope is needed so that batchesWithClippedCellsMaskSelect is released at the end.
      ...
      // Extract the edges.
      timer.Start();
      invoke(ExtractEdges<Invert>(value), batchesWithClippedCellsMaskSelect, cellBatches,
        cellBatchesData, // cellBatchesDataOffsets
        cellSet, scalars, caseIndices, edgeInterpolation);
      timer.Stop();
      log.AddDictionaryEntry("seconds-extract-edges", timer.GetElapsedTime());
    }
    ...
    invoke(GenerateCellSet<Invert>(this->EdgePointsOffset, this->CentroidPointsOffset),
      batchesWithKeptOrClippedCellsMaskSelect, cellBatches,
      cellBatchesData, // cellBatchesDataOffsets
      cellSet, caseIndices, pointMapInputToOutput, edgeInterpolationIndexToUnique, centroidOffsets,
      centroidConnectivity, this->CellMapOutputToInput, shapes, offsets, connectivity);
    timer.Stop();
    log.AddDictionaryEntry("seconds-generate-cell-set", timer.GetElapsedTime());
  ...
  public:
    ...
    VTKM_CONT void operator()(const ArrayHandleType& handle) const
    {
      ...
      *this->Result = Invert ? this->Clipper->template Run<true>(*this->CellSet, clipScalars,
                                 this->Offset, this->BatchSize, this->Log)
                             : this->Clipper->template Run<false>(*this->CellSet, clipScalars,
                                 this->Offset, this->BatchSize, this->Log);
    }
        ...
        ++component)
      {
        ...
        for (vtkm::IdComponent i = 1; i < numValues; ++i)
        {
          ...
          sum = static_cast<CType>(
            sum + VTraits::GetComponent(outputField.Get(centroid[i]), component));
        }
